...
/**************************************************************/
/* NAME :  Class HASHCH FreqPrint() */
/* DESCRIPTION :  printing the frequencies normalized with the scaling factor of the k-mers*/
/**************************************************************/ 

template <class A_Type> void HASHCH<A_Type>::FreqKmerPrintRPM(ofstream& fout,const int window, unsigned int reads){

char buffer[MAXSIZE];
double SF=(double)reads/(double) NGS;//Scaling Factor
...
for (unsigned int i=0;i<NumEl;i++){
  ...
  fout<<buffer<<"\t"<< (double)(ListCol[i].getFreq())/SF<< endl;
  }
}
