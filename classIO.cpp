...
namespace IOFn
{
  ...
  /**************************************************************/
  IOF::IOF(std::string ReadFname1,std::string fileExt1,std::string ReadFname2,std::string fileExt2,const std::string& OFname,const int& window,const int& files1,const int& files2){
...
//  for (int i=0;i<files1;i++)  
  ...
  for (int i=l;i<=u;i++)
  {
    ...
    if(!in) 
    {
      ...
      cerr << "Please check if the file path is a valid system path and the file name is correct\n"<<endl;
      exit(EXIT_FAILURE);
...
bool IOF::ReadB(){
  ...
  if (pFile==NULL)
  {
    ...
    cout<<"\n*****Starting hash table creation*****" << endl;
    return false;
...
//To cope with N    
  ...
  if (Nnum>1)  
    ...
    it1->second.frequencyNorm=it1->second.frequency; 
//To cope with N   
#endif  
  if (it1->second.frequency<TFREQ){
  auto it2=it1;
  ...
  read_signature.erase(it2);  
    ...
    //Clusterfrequency must be inizialized!!!  
    it1->second.clusterFrequency=it1->second.frequency;  
    it1++;
    }
...
#if FREQCLUSTER        
          it1->second.frequencyNorm += it2->second.frequencyNorm; //OLD VERISION
#endif 
...
#endif	  
#if FREQCLUSTER      
        it1->second.frequency += it2->second.frequency; //OLD VERSION 
#else       
        it1->second.clusterFrequency += it2->second.clusterFrequency;
#endif	  
	  auto it3=it2;
	  ++it2;
	  read_signature.erase(it3);
...
#if FREQCLUSTER        
      it2->second.frequency += it1->second.frequency; //OLD VERSION

    #if  Nnormalized        
//To cope with N         
      it2->second.frequencyNorm += it1->second.frequencyNorm; //OLD VERSION
//To cope with N 
    #endif
#else
      
     it2->second.clusterFrequency  +=it1->second.clusterFrequency; //NEW VERSION
#endif     
      auto it3=it1;
      ++it1;
	read_signature.erase(it3);
...
//merge subsignature 
  for (auto it=read_signature.begin(); it!=read_signature.end(); ++it){
  //  if (it->second.clusterFrequency>TFREQ)
#if FREQCLUSTER 
       out<<it->first<<" "<<it->second.frequency<<" "<<it->second.read<<endl; //OLD VERSION
#else       
       out<<it->first<<" "<<it->second.clusterFrequency<<" "<<it->second.read<<endl;  //NEW VERSION
#endif       
  }
  }
}
...
void IOF::FreqKmerPrint(){
  ...
  #if RPM
  sh.FreqKmerPrintRPM(outfreq,window,reads);
  #else
