...
class Signature 
{
  ...
  //
  unsigned int clusterFrequency;
#if Nnormalized  
   //It encodes the signature frequency normalized by the number of N bases
  unsigned int frequencyNorm {0};
#endif  
  Signature():r_signature{""},read{""},frequency{0},clusterFrequency{0}{};
  Signature(string r_sig, string r, unsigned int freq):r_signature{r_sig},read{r},frequency{freq},clusterFrequency{freq}{};
  Signature(char r_sig[], char r[], unsigned int freq):r_signature{string(r_sig)},read{string(r)},frequency{freq},clusterFrequency{freq}{};
  Signature(const class Signature& B):r_signature{B.r_signature},read{B.read},frequency{B.frequency},clusterFrequency{B.clusterFrequency}{};
};
#endif
...
public:
      ...
      IOF(std::string ReadFname1,std::string fileExt1,std::string ReadFname2,std::string fileExt2,const std::string& OFname,const int& window,const int& file1,const int& file2);
